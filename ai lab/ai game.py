    ...
    def plot(self, text):
        ...
        for i in range(self.length):
            label = ax.annotate(str(self.arr[i]), xy=(
                self.circle_position[i], 1), fontsize=12, ha="center")
...
def selectOption(arr, last_selected, ui, side='x'):
    ...
    if selectFormLeftOrRight == 'l':
        if arr[0] > last_selected:
            ...
            print(
                'Invalid selection you have to select a number greater than last selected')
        ...
        print('Invalid selection you have to select a number greater than last selected')
        return selectOption(arr, last_selected, ui)
